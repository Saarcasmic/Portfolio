...
const TechBadge = ({ children }) => (
  <span className="inline-block px-3 py-1.5 mr-2 mb-2 text-xs font-bold uppercase tracking-wider border border-[#333] bg-[#111] text-gray-300 hover:border-[#D64000] hover:text-white transition-colors cursor-default">
    {children}
  </span>
);

const SectionHeader = ({ title, icon: Icon }) => (
  <div className="flex items-center gap-4 mb-8 border-b border-[#222] pb-4 mt-16">
    <div className="p-2 bg-[#111] rounded-lg border border-[#222]">
      <Icon size={24} className="text-[#D64000]" />
    </div>
    <h3 className="text-3xl font-display font-bold text-white uppercase tracking-tight">{title}</h3>
  </div>
);

const LargeExperienceCard = ({ role, company, date, bullets, location }) => (
  <div className="relative pl-8 md:pl-0 mb-12 last:mb-0 group">
    ...
    <div className="bg-[#111] border border-[#222] p-8 rounded-xl hover:border-[#D64000]/50 transition-colors duration-300">
      <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-6 gap-4">
        <div>
          <h4 className="text-2xl font-bold text-white mb-1 group-hover:text-[#D64000] transition-colors">{company}</h4>
          <div className="flex items-center gap-3 text-[#D64000] font-medium text-lg">
            <span>{role}</span>
          </div>
        </div>
        <div className="text-right">
          <div className="text-sm font-mono text-gray-400 bg-[#1a1a1a] px-3 py-1 rounded inline-block mb-1 border border-[#333]">{date}</div>
          {location && <div className="text-xs text-gray-500 font-mono flex items-center justify-end gap-1"><MapPin size={12} /> {location}</div>}
        </div>
      </div>

      <ul className="space-y-3">
        {bullets.map((point, i) => (
          <li key={i} className="flex gap-3 text-base text-gray-300 leading-relaxed font-light">
            <span className="text-[#D64000] mt-2 h-1.5 w-1.5 rounded-full flex-shrink-0 bg-[#D64000]"></span>
            {point}
          </li>
        ))}
      </ul>
...
const LargeEducationCard = ({ school, degree, date, grade, bullets }) => (
  <div className="flex flex-col md:flex-row gap-6 p-6 border-b border-[#222] hover:bg-[#111] transition-colors rounded-lg">
    <div className="md:w-1/4">
      <span className="text-sm font-mono text-[#D64000] block mb-1">{date}</span>
    ...
    <div className="md:w-3/4">
      ...
      <p className="text-base text-gray-300 font-medium mb-2">{degree}</p>
...
const SkillCategory = ({ title, skills, icon: Icon }) => (
  <div className="bg-[#111] p-6 border border-[#222] rounded-lg h-full hover:border-gray-700 transition-colors">
    <div className="flex items-center gap-3 mb-4">
      <Icon size={18} className="text-[#D64000]" />
...
const AchievementCard = ({ title, subtitle, icon: Icon, link: Link }) => (
  <a href={Link} target="_blank" rel="noopener noreferrer">
    <div className="flex items-start gap-4 p-4 border border-[#222] bg-[#0a0a0a] rounded-lg hover:border-[#D64000] transition-colors group">
      <div className="p-3 bg-[#111] rounded group-hover:bg-[#222] transition-colors">
        <Icon size={24} className="text-[#D64000]" />
      </div>
      <div>
...
const ProjectCard = ({ title, desc, tech, index, aiFeature, link, image }) => {
  ...
  return (
    <div
      className="group relative border-t border-[#222] hover:border-[#D64000] transition-colors duration-500 py-12"
      style={{ animation: `fadeInUp 0.8s ease-out ${index * 0.1}s backwards` }}
    >
      <div className="flex flex-col md:flex-row gap-8">
        <div className="md:w-1/3">
          {image && (
            <a href={link} target="_blank" rel="noopener noreferrer" className="block mb-6 overflow-hidden rounded-lg border border-[#222] group-hover:border-[#D64000] transition-colors">
              <img
                src={image}
                alt={title}
          ...
          <a href={link} target="_blank" rel="noopener noreferrer" className="block">
            <h3 className="text-3xl font-bold font-display text-white group-hover:translate-x-2 transition-transform duration-300 flex items-center gap-3">
              {title}
              <ArrowUpRight className="opacity-0 group-hover:opacity-100 transition-opacity text-[#D64000]" size={24} />
            </h3>
          </a>
          <div className="flex flex-wrap gap-2 mt-4">
            {tech.map((t, i) => (
              <span key={i} className="text-xs font-mono text-[#666]">/{t}</span>
            ))}
          </div>
...
const AnimatedStatTile = ({ value, label, suffix = '', delay = 0 }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-[#111] border border-[#222] p-6 rounded-xl hover:border-[#D64000]/50 transition-all duration-300 group cursor-default"
    >
      <span className="text-3xl md:text-4xl font-display font-bold text-white group-hover:text-[#D64000] transition-colors">
        {formattedValue}{suffix}
      </span>
      <span className="block text-xs font-mono uppercase tracking-widest text-gray-500 mt-2">{label}</span>
    </motion.div>
  );
};
...
const FreelanceProjectCard = ({ clientName, image, link }) => (
  <div className="group relative bg-[#111] border border-[#222] rounded-lg overflow-hidden hover:border-[#D64000] transition-all duration-300">
    ...
    <div className="p-4">
      <h4 className="text-lg font-bold text-white mb-2">{clientName}</h4>
      {link && (
        <a
          href={link}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 text-sm text-[#D64000] hover:text-white font-medium transition-colors group/link"
        >
          View Project
...
const ExperienceSection = () => {
  ...
  return (
    <div className="py-12 border-t border-b border-[#222] bg-[#0a0a0a]">
      <div className="max-w-6xl mx-auto flex flex-col gap-10 items-center text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 w-full justify-items-center items-start">
          ...
          <div className="flex flex-col items-center gap-6 w-full max-w-xl">
            ...
            <div className="flex flex-col gap-6 w-full">
              {companies.map((company, i) => (
                <div
                  key={i}
                  className="w-full max-w-xl flex items-center gap-6 group cursor-default mx-auto flex-nowrap mt-10"
                >
                  <div className="relative w-20 h-20 md:w-24 md:h-24 flex items-center justify-center bg-[#111] border border-[#222] rounded-lg p-3 hover:border-[#D64000] transition-all duration-300 group-hover:scale-105 flex-shrink-0">
                    <img
                      src={company.logo}
                      alt={company.alt}
                  ...
                  <span className="text-sm md:text-base font-mono uppercase tracking-widest text-gray-400 group-hover:text-white transition-colors whitespace-nowrap">
                    {company.name}
                  </span>
                  <span className="text-xs md:text-sm font-mono uppercase tracking-widest text-gray-500 group-hover:text-white transition-colors whitespace-nowrap ml-auto">
                    {company.time}
                  </span>
                </div>
              ))}
...
const ProjectShowcaseCard = ({ project, onTryDemo }) => (
  <div className="relative h-full bg-[#111] border border-[#222] rounded-2xl overflow-hidden group">
    {/* Project Image */}
    <div className="relative h-48 md:h-64 overflow-hidden">
      ...
      <div className="absolute top-4 left-4 flex flex-wrap gap-2">
        {project.tech.slice(0, 2).map((t, i) => (
          <span key={i} className="px-2 py-1 text-xs font-mono bg-black/60 backdrop-blur-sm text-white rounded">
            {t}
          </span>
        ))}
      </div>
    ...
    {/* Content */}
    <div className="p-6">
      <h3 className="text-2xl font-display font-bold text-white mb-2 group-hover:text-[#D64000] transition-colors">
        {project.title}
      </h3>
      ...
      <div className="flex gap-3">
        {project.tryLink && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#D64000] text-white text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-[#b03500] transition-colors"
          >
            <Sparkles size={14} /> Try It
          </button>
        )}
        <a
          href={project.link}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-4 py-2 border border-[#333] text-gray-300 text-xs font-bold uppercase tracking-wider rounded-lg hover:border-white hover:text-white transition-colors"
        >
          <Github size={14} /> Code
        </a>
      </div>
  ...
  return (
    <div className="relative isolate min-h-screen flex flex-col justify-start pt-4 md:pt-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8">
        ...
        >
          <div className="relative bg-[#0a0a0a] border border-[#222] rounded-2xl p-4 overflow-hidden">
            {/* Swiper Navigation */}
            <div className="absolute top-4 right-4 z-20 flex gap-2">
              <button
                onClick={() => swiperRef.current?.slidePrev()}
                className="p-2 bg-[#111] border border-[#333] rounded-lg text-gray-400 hover:text-white hover:border-[#D64000] transition-colors"
              >
                <ChevronLeft size={20} />
              </button>
              <button
        ...
        >
          <div className="h-full bg-[#111]/80 backdrop-blur-xl border border-[#222] rounded-2xl p-8 flex flex-col justify-between relative overflow-hidden">
            {/* Decorative gradient */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#D64000]/10 rounded-full blur-3xl" />
            
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-6">
                <span className="w-3 h-3 bg-[#D64000] rounded-full animate-pulse" />
                <span className="text-xs font-mono uppercase tracking-widest text-[#D64000]">Available for work</span>
              </div>
              ...
              <h1 className="text-4xl md:text-5xl font-display font-bold text-white mb-2 leading-tight">
                ...
                <img
                  ...
                  alt="Profile"
                  className="w-40 h-40 rounded-full object-cover border-4 border-[#222] shadow-lg"
                />
              </div>
              

              <p className="text-sm font-mono uppercase tracking-widest text-gray-400 mt-4 mb-6">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row gap-3">
              <button
                ...
                className="group flex items-center justify-center gap-3 px-6 py-3 bg-[#D64000] text-white text-sm font-bold uppercase tracking-wider rounded-lg hover:bg-[#b03500] transition-all"
              >
                View All Work
              ...
              <button
                ...
                className="flex items-center justify-center gap-3 px-6 py-3 border border-[#333] text-gray-300 text-sm font-bold uppercase tracking-wider rounded-lg hover:border-white hover:text-white transition-all"
              >
                Let's Talk
              </button>
            </div>
...
const AboutPage = () => (
  <div className="animate-in fade-in slide-in-from-bottom-10 duration-700 max-w-5xl mx-auto">
    ...
    <div className="bg-[#080808]/85 backdrop-blur-md rounded-2xl p-8 md:p-12 border border-[#222]/50">
    <div className="flex items-end gap-4 mb-12 border-b border-[#333] pb-8">
      <h2 className="text-6xl font-display font-bold text-white uppercase tracking-tighter">About Me</h2>
      <span className="h-4 w-4 bg-[#D64000] mb-4"></span>
    </div>
    ...
    <div className="mb-16">
      ...
      <div className="mt-8">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 bg-[#D64000] text-white px-6 py-3 text-sm font-bold uppercase tracking-widest hover:bg-[#b03500] transition-colors rounded-lg"
        >
    ...
    <div className="animate-in fade-in slide-in-from-bottom-10 duration-700">
      ...
      <div className="bg-[#080808]/85 backdrop-blur-md rounded-2xl p-8 md:p-12 border border-[#222]/50">
        <div className="flex items-end gap-4 mb-20">
          <h2 className="text-6xl font-display font-bold text-white uppercase tracking-tighter">Work</h2>
          <span className="h-4 w-4 bg-[#D64000] mb-4"></span>
        </div>
...
const ContactPage = () => {
  ...
  return (
    <div className="min-h-[60vh] flex flex-col justify-between animate-in fade-in slide-in-from-bottom-10 duration-700">
      {/* Content Overlay for better readability */}
      <div className="bg-[#080808]/85 backdrop-blur-md rounded-2xl p-8 md:p-12 border border-[#222]/50">
      <div className="grid md:grid-cols-2 gap-16">
        <div>
          <h2 className="text-[6vw] leading-[0.8] font-display font-bold text-white uppercase tracking-tighter mb-12">
            Let's<br />Talk.
          </h2>

          <div className="border-t border-[#333] pt-12">
            <div className="mb-8">
              <p className="text-gray-400 text-sm mb-2 uppercase tracking-widest">Email</p>
              <a
                href="mailto:agrawalsaar16@gmail.com"
                className="text-2xl text-white font-bold hover:text-[#D64000] transition-colors border-b-2 border-[#D64000] pb-1 inline-block"
              >
            ...
            <div>
              <p className="text-gray-400 text-sm mb-2 uppercase tracking-widest">Phone</p>
              <p className="text-xl text-white font-bold">+91 8791567123</p>
            </div>
          </div>
        </div>

        {/* Quick Info Section */}
        <div className="bg-[#111] p-8 border border-[#222] self-start rounded-lg">
          <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
            <Briefcase className="text-[#D64000]" size={20} />
            Quick Info
          </h3>
      ...
      <div className="grid md:grid-cols-2 gap-12 mt-12 pt-12 border-t border-[#333]">
        <div className="flex flex-col gap-4 items-start">
          <a href="https://linkedin.com/in/saarcasmic" className="text-gray-500 hover:text-white uppercase tracking-widest text-sm font-bold transition-colors flex items-center gap-2 group">
            <span className="w-2 h-2 bg-[#333] group-hover:bg-[#D64000] transition-colors"></span> LinkedIn
          </a>
          <a href="https://github.com/saarcasmic" className="text-gray-500 hover:text-white uppercase tracking-widest text-sm font-bold transition-colors flex items-center gap-2 group">
            <span className="w-2 h-2 bg-[#333] group-hover:bg-[#D64000] transition-colors"></span> GitHub
          </a>
          <a href="https://leetcode.com/saarcasmic" className="text-gray-500 hover:text-white uppercase tracking-widest text-sm font-bold transition-colors flex items-center gap-2 group">
...
const App = () => {
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-[#080808] border-t border-[#222] z-50 md:hidden flex justify-around p-4">
        <button onClick={() => setPage('home')} className={`text-xs uppercase font-bold tracking-widest ${page === 'home' ? 'text-[#D64000]' : 'text-gray-500'}`}>Home</button>
        <button onClick={() => setPage('about')} className={`text-xs uppercase font-bold tracking-widest ${page === 'about' ? 'text-[#D64000]' : 'text-gray-500'}`}>About</button>
        <button onClick={() => setPage('projects')} className={`text-xs uppercase font-bold tracking-widest ${page === 'projects' ? 'text-[#D64000]' : 'text-gray-500'}`}>Work</button>
        <button onClick={() => setPage('contact')} className={`text-xs uppercase font-bold tracking-widest ${page === 'contact' ? 'text-[#D64000]' : 'text-gray-500'}`}>Contact</button>
      </nav>
      ...
      {/* Main Content Area */}
      <main className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-16 min-h-screen flex flex-col justify-center border-l border-[#111] border-r border-[#111]">
